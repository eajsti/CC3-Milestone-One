...
class ParkingSessionService {
    ...
    public int startSession(int vehicleId, int zoneId) {
        try (Connection c = DBConnection.connect()) {
            PreparedStatement activePs = c.prepareStatement(
                    "SELECT s.Id, sl.Id AS SlotId, z.Name AS ZoneName " +
                            "FROM Sessions s " +
                            "JOIN Slots sl ON s.SlotId = sl.Id " +
                            "JOIN Zones z ON sl.ZoneId = z.Id " +
                            "WHERE s.VehicleId = ? AND s.End IS NULL");
            activePs.setInt(1, vehicleId);
            ResultSet activeRs = activePs.executeQuery();
            if (activeRs.next()) {
                int currentSlot = activeRs.getInt("SlotId");
                String currentZone = activeRs.getString("ZoneName");
                System.out.println("You are already parked in Zone " + currentZone + ", Slot " + currentSlot + ".");
                return -1;
            }
